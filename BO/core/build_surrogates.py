...
class SurrogateBuilder:
    ...
    def train_quantile_regressor(
        ...
        # Train three models: lower (0.25), median (0.5), upper (0.75)
        # Note: Changed quantiles from (0.1, 0.5, 0.9) to (0.25, 0.5, 0.75)
        # for more stable regression on heavily skewed FNR distribution
        quantile_models = {}
        for alpha, name in [(0.25, 'lower'), (0.5, 'median'), (0.75, 'upper')]:
            model = GradientBoostingRegressor(
                n_estimators=150,  # Reduced from 200 to avoid overfitting
                learning_rate=0.05,
                max_depth=5,  # Reduced from 6 for simpler model
                subsample=0.8,
                min_samples_split=10,  # Increased from 5 to reduce noise sensitivity
    ...
    def train_all_surrogates(
        ...
        self.logger.info("=" * 80)
        self.logger.info("TRAINING SURROGATE MODELS (Scientific Redesign)")
        self.logger.info("=" * 80)
        ...
        n_dr_binary = np.sum((y_dr == 0) | (y_dr == 1))
        if n_dr_binary / len(y_dr) > 0.95:
            self.logger.warning(f"  [WARNING] DR is {n_dr_binary/len(y_dr)*100:.1f}% binary - classifier may be more appropriate than regressor")

        n_fnr_binary = np.sum((y_fnr == 0) | (y_fnr == 1))
        if n_fnr_binary / len(y_fnr) > 0.90:
            self.logger.warning(f"  [WARNING] FNR is {n_fnr_binary/len(y_fnr)*100:.1f}% binary - quantile regression may struggle")
            self.logger.warning(f"  [WARNING] Consider using classification approach for FNR in future improvements")

        fnr_zero_pct = np.sum(y_fnr == 0) / len(y_fnr) * 100
        if fnr_zero_pct > 80:
            self.logger.warning(f"  [WARNING] {fnr_zero_pct:.1f}% of FNR values are 0.0 (extreme class imbalance)")
    ...
    def save_surrogates(self, output_dir: Path, version: str = 'v1'):
        ...
        metadata = {
            'version': version,
            'version_description': 'Calibrated classifier for DR, quantile regression for FNR/TTD',
            'n_features': 5,
        ...
        self.logger.info(f"\n[OK] All surrogates and preprocessing state saved to {saved_ml_dir}")
