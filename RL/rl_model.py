...
class DualLogger:
        ...
        class TeeWriter:
            ...
            def write(self, message):
                ...
                try:
                    ...
                    # Encode to terminal's encoding, ignoring problematic characters

                    safe_msg = message.encode(self.terminal.encoding or 'utf-8', errors='ignore').decode(self.terminal.encoding or 'utf-8', errors='ignore')
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        print(f"[Log] Log saved to: {self.log_file}")
...
class EnsembleModelV2:
    ...
    Improved ensemble with log-space handling for TTD.

    Used for time_to_detection_threshold which has heavy-tailed distribution.

    MUST be at module level for pickle serialization!

    """

    def __init__(self, lgb_m, xgb_m, cb_m, use_log=False, clip_range=None):
    ...
    def predict(self, X):
        ...
        lgb_p = self.lgb.predict(X)

        xgb_p = self.xgb.predict(X)

        cb_p = self.cb.predict(X)

        pred = 0.3*lgb_p + 0.3*xgb_p + 0.4*cb_p
...
class EnsembleModelV3:

    """

    Optimized FNR ensemble with CatBoost.

    MUST be at module level for pickle serialization!

    """

    def __init__(self, lgb_m, xgb_m, cb_m):
    ...
    def predict(self, X):
        ...
        pred = 0.25*lgb_p + 0.25*xgb_p + 0.5*cb_p
...
class TwoStageTTDPredictor:
    ...
    OK COMPLETELY REDESIGNED TTD predictor with physics-informed approach
...
class BiosensorPipeline:
    ...
    def __init__(self, data_path: str, output_dir: str = "biosensor_results"):
        ...
        # OK Initialize surrogate_features dictionary early
    ...
    def validate_no_nan_inf(arr, name="array"):
        ...
        if has_nan or has_inf:
            ...
            print(f"      [WARNING] WARNING: {name} contains NaN={has_nan}, inf={has_inf}")
            ...
            if col in X_encoded.columns:
                ...
                if X_encoded[col].dtype == 'object' or X_encoded[col].dtype.name == 'category':
                    ...
                    if X_encoded[col].isnull().any():
                        ...
                        print(f"      [WARNING] WARNING: {col} has unmapped values: {unmapped}")
                    ...
                    print(f"      OK Encoded {col}: {list(mapping.keys())}")
    ...
    def load_data(self, biomarker: Optional[str] = None) -> pd.DataFrame:
        ...
        print(f"        Loading Sclerostin simulation dataset: {self.data_path}")
        ...
        print(f"[*] Master index loaded: {master_df.shape} simulations")

        

        # Extract features from metadata JSON files

        print("       Extracting biosensor configurations from metadata files...")
        ...
        for idx, row in master_df.iterrows():
            ...
            try:
                ...
                # OK NEW: Physics-Informed Features for TTD Prediction
                ...
                # OK CRITICAL: Biosensor-Environment Interaction Features
        ...
        print(f"[*] Extracted features from {len(features_list)} simulations")
        ...
        if missing_targets:
            ...
            print(f"             WARNING: Missing target metrics: {missing_targets}")

        else:

            print(f"[*] All target metrics available: {available_targets}")
        ...
        print(f"Multi-objective score calculated: {self.raw_data['multi_objective_score'].describe()}")
    ...
    def preprocess_data(self, test_size: float = 0.2, 
        ...
        print(f"\n        VALIDATION: Feature Classification")
        ...
        if len([f for f in available_modifiable if f in X_final.columns]) == 0:
            ...
            print(f"               WARNING: No modifiable features found in final dataset!")
    ...
    def augment_training_data(self, augmentation_factor=3):  # OK 3x, not 10x

        """Simple, effective augmentation with SMOTE-like interpolation"""

        print(f"[DATA] Augmenting training data by {augmentation_factor}x...")
        ...
        for i in range(n_samples_to_generate):
            ...
            alpha = np.random.uniform(0.3, 0.7)  # OK Stay closer to real data
        ...
        print(f"OK Augmented: {len(self.X_train)} samples (from {len(X_modifiable)})")
    ...
    def add_noise_augmentation(self, noise_level=0.02):
        ...
        print(f"  Adding noise augmentation ( ={noise_level})...")
        ...
        print(f"OK Added {len(X_train_noisy)} noisy samples")
    ...
    def plot_target_metrics_distributions(self, save_folder: str = "target_metric_plots"):
        ...
        for i, metric in enumerate(self.output_metrics):
            ...
            if metric_data.dtype == bool:
                ...
                print(f"             Skipping {metric} - boolean data type not suitable for distribution plots")
        ...
        for col in self.y_train.columns:
            ...
            if self.y_train[col].dtype in ['float64', 'int64', 'float32', 'int32']:
                ...
                if len(train_data) < 10:
                    ...
                    print(f"             Skipping {col} - insufficient valid data")
                ...
                if not valid_transformations:
                    ...
                    # OK More lenient transformation criteria
                    ...
                    # OK Use transformation if it improves skewness by ANY amount AND R  gain is expected
                    ...
                    # OK FIX: Add handlers for RobustScale and MinMaxScale
                    ...
                    # OK FIX: Add else clause to catch any unexpected transform names

                    else:

                        # Fallback: use raw values if transform name is unexpected

                        print(f"      [WARNING]  WARNING: Unknown transform '{best_transform['transform']}', using raw values")
    ...
    def _create_advanced_features(self, X):
        ...
        # OK ONLY add these 3 proven features:
    ...
    def train_supervised_models(self, target_metric: str = 'multi_objective_score') -> Dict:
        ...
        if target_metric not in self.y_train.columns:
            ...
            print(f"            Target metric '{target_metric}' not found. Using first available: {self.y_train.columns[0]}")
        ...
        # OK Use RepeatedKFold for more robust estimates
        ...
        if best_performance['test_r2'] < 0.5:
            ...
            print("             WARNING: Best model R     < 0.5. Consider:")
            ...
            print("             MODERATE: Best model R     < 0.7. Room for improvement.")
    ...
    def _train_neural_network_cv(self, X_train, y_train, X_val, y_val, model_name, target_metric, fold):
        ...
        try:
            ...
            print(f"                   Metric calculation failed, using scaled targets: {e}")
    ...
    def _create_robust_mlp_model(self, input_dim, hidden_dims=None):
        ...
        OK FIXED: Uses module-level RobustMLP class for pickle support

        """

        # OK Use module-level RobustMLP class (defined at top of file)
    ...
    def _create_adaptive_ensemble_model(self, input_dim, task_type='regression', n_classes=None):
        ...
        OK FIXED: Uses module-level AdaptiveEnsemble class for pickle support

        """

        # OK Use module-level AdaptiveEnsemble class (defined at top of file)
    ...
    def _train_neural_network_regressor(self, X_train, y_train, X_test, y_test, target_name):
        ...
        OK FIXED: Uses module-level TabularNN and NNWrapper classes for pickle support
        ...
        # OK Create validation split (use test set for validation during training)
        ...
        # OK Use module-level TabularNN class (defined at top of file)
        ...
        print(f"         OK Neural Network: Train R ={train_r2:.3f}, Test R ={test_r2:.3f}")

        

        # OK Use module-level NNWrapper class (defined at top of file)
    ...
    def _train_with_multiple_strategies(self, X_train_final, y_train_processed, X_test_final, y_test_processed, 
        ...
        print(f"      [TARGET] Training high-performance ensemble...")
        ...
        print(f"      [DATA] Ensemble weights: XGB={meta_learner.coef_[0]:.3f}, LGB={meta_learner.coef_[1]:.3f}, RF={meta_learner.coef_[2]:.3f}")
        ...
        print(f"      OK Best: {best_name} (R  = {best_r2:.4f})")
    ...
    def _create_enhanced_traditional_model(self, input_dim, task_type='regression', n_classes=None):
        ...
        OK FIXED: Uses module-level EnhancedTraditional class for pickle support

        """

        # OK Use module-level EnhancedTraditional class (defined at top of file)
    ...
    def _fix_index_alignment(self, X_data, y_data, target_metric):
        ...
        try:
            ...
            print(f"                   Index alignment failed, using provided y_data: {e}")

            return y_data



    def _engineer_biosensor_features(self, df):

        """

        Engineer physics-based features for biosensor modeling.

        

        These features capture known relationships:

        - Signal quality (SNR transformations)

        - Detection efficiency (SNR   dynamic range)

        - Expression rates (log-transformed)
        ...
        if 'signal_to_noise_ratio_SNR' in df.columns:

            # SNR in dB scale (common in signal processing)

            df['SNR_dB'] = 10 * np.log10(df['signal_to_noise_ratio_SNR'].clip(lower=1e-6))

            

            # SNR quality categories (captures non-linear effects)

            df['SNR_quality'] = pd.cut(

                df['signal_to_noise_ratio_SNR'],
            ...
            # Squared SNR (signal power)

            df['SNR_squared'] = df['signal_to_noise_ratio_SNR'] ** 2

        

        #     Detection Features    

        if 'signal_to_noise_ratio_SNR' in df.columns and 'dynamic_range_of_output' in df.columns:

            # Detection efficiency (higher SNR + higher range = faster/better detection)

            df['detection_efficiency'] = (

                df['signal_to_noise_ratio_SNR'] * df['dynamic_range_of_output']
            ...
            df['signal_strength'] = (
                ...
                np.sqrt(df['signal_to_noise_ratio_SNR']) * df['dynamic_range_of_output']
            ...
            df['detectability'] = np.log1p(df['detection_efficiency'])

        

        #     Expression Rate Features    

        if 'transcription_rate' in df.columns:

            # Log-transform (rates are often log-normal)

            df['log_transcription_rate'] = np.log1p(df['transcription_rate'])

        

        if 'translation_rate' in df.columns:

            df['log_translation_rate'] = np.log1p(df['translation_rate'])

        

        if 'transcription_rate' in df.columns and 'translation_rate' in df.columns:

            # Expression efficiency

            df['expression_efficiency'] = (

                df['transcription_rate'] * df['translation_rate']

            )

            df['log_expression_efficiency'] = np.log1p(df['expression_efficiency'])

            

            # Expression balance

            df['transcription_translation_ratio'] = (

                df['transcription_rate'] / (df['translation_rate'] + 1e-6)

            )

        

        #     Cooperativity Features    

        if 'hill_coefficient' in df.columns:

            # Ultrasensitivity indicator (Hill > 2)

            df['is_ultrasensitive'] = (df['hill_coefficient'] > 2).astype(float)

            

            # Cooperativity strength (deviation from Hill=1)

            df['cooperativity_strength'] = np.abs(df['hill_coefficient'] - 1)

            

            # Sigmoid steepness

            df['sigmoid_steepness'] = df['hill_coefficient'] ** 2

        

        #     Binding Features    

        if 'kd_value' in df.columns:

            # Log Kd (binding affinity in log space)

            df['log_kd'] = np.log1p(df['kd_value'])

            

            # Binding strength (1/Kd)

            df['binding_strength'] = 1 / (df['kd_value'] + 1e-6)

        

        #     Stability Features    

        if 'mrna_degradation_rate' in df.columns and 'protein_degradation_rate' in df.columns:

            # Total degradation

            df['total_degradation'] = (

                df['mrna_degradation_rate'] + df['protein_degradation_rate']
            ...
            df['stability_ratio'] = (
                ...
                df['protein_degradation_rate'] / (df['mrna_degradation_rate'] + 1e-6)

            )

            

            # Half-life proxy (1/degradation)

            df['mrna_halflife_proxy'] = 1 / (df['mrna_degradation_rate'] + 1e-6)

            df['protein_halflife_proxy'] = 1 / (df['protein_degradation_rate'] + 1e-6)

        

        #     Interaction Features    

        if 'hill_coefficient' in df.columns and 'kd_value' in df.columns:

            # Response sharpness (cooperativity   affinity)

            df['response_sharpness'] = df['hill_coefficient'] * np.log1p(1 / (df['kd_value'] + 1e-6))
        ...
        if new_features > 0:
            ...
            print(f"      OK Engineered {new_features} new physics-based features")
    ...
    def train_surrogate_models(self, force_retrain=False):
        ...
        for col in self.y_train.columns:
            ...
            if 'false_negative_rate' in col and y_max > 1.0:
                ...
                raise ValueError(f"ERROR BUG: {col} has max={y_max:.6f} > 1.0! Wrong data!")

            if 'time_to_detection' in col and y_max > 100:

                print(f"   [WARNING] WARNING: {col} has max={y_max:.2f} > 100 (unusual but might be OK)")
        ...
        self.y_scalers = {}  # OK CRITICAL FIX: Initialize y_scalers for neural networks

        surrogate_performance = {}

        

        print("OK Initialized y_scalers dictionary for neural network target scaling")

        

        # OK ADD VALIDATION: Ensure dictionaries stay initialized

        print(f"[DATA] Initialized storage:")
        ...
        if not hasattr(self, 'X_train_scaled') or self.X_train_scaled is None:
            ...
            print("    Scaled features not found, preparing data...")
        ...
        if hasattr(self, 'X_train_enhanced') and self.X_train_enhanced is not None:
            ...
            print("[DATA] Using enhanced feature set for surrogate models")
            ...
            print("[DATA] Using scaled feature set for surrogate models")
        ...
        # Select surrogate features (exclude target_metric and other targets)

        feature_columns = [col for col in self.X_train.columns 

                            if col not in self.target_metrics]

        

        X_train_surrogate = self.X_train[feature_columns].copy()

        X_test_surrogate = self.X_test[feature_columns].copy()

        

        # [WARNING] CRITICAL FIX: DO NOT apply feature engineering here!

        # Reason: Engineered features (is_ultrasensitive, cooperativity_strength, etc.)

        # are created during training but NOT available during RL inference.

        # This causes "Feature not found" warnings during RL step() calls.

        # Instead, train surrogates on raw features that ARE available in RL.

        print(f"\n       Using raw features for surrogate training (no engineering)")

        print(f"      This ensures compatibility with RL environment inference")

        print(f"      Surrogate X_train shape: {X_train_surrogate.shape}")
        ...
        self.surrogate_feature_scaler.fit(X_train_surrogate)  # Fit to already-scaled data
        ...
        for target_idx, target_metric in enumerate(target_order):
            ...
            print(f"\n[TARGET] Target {target_idx + 1}/{len(self.y_train.columns)}: {target_metric}")
            ...
            print(f"   y_train: min={y_train_target.min():.6f}, max={y_train_target.max():.6f}, mean={y_train_target.mean():.6f}, std={y_train_target.std():.6f}")

            print(f"   y_test: min={y_test_target.min():.6f}, max={y_test_target.max():.6f}, mean={y_test_target.mean():.6f}, std={y_test_target.std():.6f}")

            

            # OK Check for data quality issues

            if y_train_target.std() < 1e-6:

                print(f"   [WARNING] WARNING: Target has very low variance (std={y_train_target.std():.9f})")

                print(f"      This will make prediction difficult!")

            

            # Check for outliers

            q1, q99 = y_train_target.quantile(0.01), y_train_target.quantile(0.99)

            outlier_frac = ((y_train_target < q1) | (y_train_target > q99)).mean()

            if outlier_frac > 0.1:

                print(f"   [WARNING] WARNING: {outlier_frac*100:.1f}% outliers detected")
            ...
            if target_metric in expected_ranges:
                ...
                if actual_min < exp_min or actual_max > exp_max:
                    ...
                    print(f"   [DATA] INFO: Values slightly outside typical range [{exp_min}, {exp_max}]")
                    ...
                    print(f"   OK Values within expected range [{exp_min}, {exp_max}]")
            ...
            if 'false_negative_rate' in target_metric:
                ...
                if y_train_target.max() > 1.0:
                    ...
                    raise ValueError(f"ERROR CRITICAL BUG: FNR has values > 1.0 (max={y_train_target.max():.6f})!")
                ...
                if y_train_target.max() > 0.3:
                    ...
                    print(f"   [WARNING] WARNING: FNR max={y_train_target.max():.4f} is unusually high")
                ...
                if y_train_target.min() < 0:
                    ...
                    raise ValueError(f"ERROR CRITICAL BUG: FNR has negative values (min={y_train_target.min():.6f})!")
            ...
            # [TARGET] ULTIMATE: Train FNR using MULTI-STRATEGY approach (like TTD)
            ...
            if target_metric == 'false_negative_rate' and not is_classification:
                ...
                # APPROACH 1: LightGBM + XGBoost + CatBoost Ensemble (FNR-Optimized)
                ...
                try:
                    ...
                    lgb_params = {
                        ...
                        'num_leaves': 63,  # Slightly larger for better fitting

                        'learning_rate': 0.02,  # Slower for better convergence
                        ...
                        'min_data_in_leaf': 10,  # Prevent overfitting to small FNR values
                    ...
                    lgb_model.fit(
                        ...
                        X_train_surrogate, y_train_target.values,

                        eval_set=[(X_test_surrogate, y_test_target.values)],
                    ...
                    xgb_params = {
                        ...
                        'max_depth': 8,  # Deeper for FNR complexity
                    ...
                    # CatBoost for even better handling of mixed features
                    ...
                    cb_model.fit(X_train_surrogate, y_train_target.values, 

                                eval_set=(X_test_surrogate, y_test_target.values))

                    

                    # Ensemble: Weighted average (better balance for FNR)

                    lgb_pred = lgb_model.predict(X_test_surrogate)

                    xgb_pred = xgb_model.predict(X_test_surrogate)

                    cb_pred = cb_model.predict(X_test_surrogate)

                    

                    # Use weighted average with CatBoost getting high weight

                    weights = np.array([0.25, 0.25, 0.5])  # LGB, XGB, CB (CB is best for FNR)

                    ensemble_pred = weights[0]*lgb_pred + weights[1]*xgb_pred + weights[2]*cb_pred

                    

                    # Clip to valid FNR range [0, 1]

                    ensemble_pred = np.clip(ensemble_pred, 0, 1)

                    

                    ensemble_r2 = r2_score(y_test_target.values, ensemble_pred)

                    ensemble_mse = np.mean((y_test_target.values - ensemble_pred)**2)
                    ...
                    print(f"         True test range: [{y_test_target.values.min():.4f}, {y_test_target.values.max():.4f}]")

                    

                    # Create wrapper for ensemble

                    ensemble_model = EnsembleModelV3(lgb_model, xgb_model, cb_model)
                    ...
                    fnr_candidates['ensemble'] = {
                        ...
                        'name': 'LGB+XGB+CB Ensemble (Optimized)'

                    }

                    print(f"         OK Ensemble: R ={ensemble_r2:.3f}")
                ...
                try:
                    ...
                    nn_model, nn_r2, nn_mse = self._train_neural_network_regressor(
                        ...
                        X_train_surrogate, y_train_target.values,  # Use ORIGINAL targets

                        X_test_surrogate, y_test_target.values,
                    ...
                    print(f"         OK Neural Network: R ={nn_r2:.3f}")
                ...
                try:
                    ...
                    # OK CRITICAL FIX: Create and use y_scaler for FNR neural network

                    from sklearn.preprocessing import StandardScaler

                    y_scaler_fnr = StandardScaler()

                    

                    # Scale targets

                    y_train_scaled = y_scaler_fnr.fit_transform(y_train_target.values.reshape(-1, 1)).flatten()

                    y_test_scaled = y_scaler_fnr.transform(y_test_target.values.reshape(-1, 1)).flatten()

                    

                    # Store y_scaler

                    if not hasattr(self, 'y_scalers'):

                        self.y_scalers = {}

                    self.y_scalers[target_metric] = y_scaler_fnr

                    

                    print(f"         OK Created y_scaler for FNR: mean={y_scaler_fnr.mean_[0]:.4f}")
                    ...
                    n_epochs = 300  # Define epochs for FNR Enhanced NN
                    ...
                    for epoch in range(n_epochs):
                        ...
                        for X_batch, y_batch in train_loader:
                            ...
                            predictions = enhanced_nn(X_batch)

                            loss = criterion(predictions, y_batch)

                            loss.backward()

                            torch.nn.utils.clip_grad_norm_(enhanced_nn.parameters(), max_norm=1.0)
                        ...
                        with torch.no_grad():
                            ...
                            val_predictions = enhanced_nn(X_test_t)
                    ...
                    with torch.no_grad():
                        ...
                        test_pred_scaled = enhanced_nn(X_test_t).numpy().flatten()

                    

                    # OK CRITICAL: Inverse transform predictions to original scale

                    if hasattr(self, 'y_scalers') and target_metric in self.y_scalers:

                        y_scaler = self.y_scalers[target_metric]
                        ...
                        # OK CRITICAL: Clip to valid FNR range [0, 1]

                        test_pred_original = np.clip(test_pred_original, 0.0, 1.0)

                        

                        from sklearn.metrics import r2_score, mean_squared_error

                        enhanced_r2 = r2_score(y_test_target.values, test_pred_original)

                        enhanced_mse = mean_squared_error(y_test_target.values, test_pred_original)

                        

                        print(f"         OK FNR predictions (original, clipped): [{test_pred_original.min():.4f}, {test_pred_original.max():.4f}]")
                        ...
                        enhanced_r2 = r2_score(y_test_target.values, test_pred_scaled)

                        enhanced_mse = mean_squared_error(y_test_target.values, test_pred_scaled)

                        print(f"         [WARNING]  Using SCALED predictions (no y_scaler found)")
                    ...
                    print(f"         OK Enhanced NN: R ={enhanced_r2:.3f}")

                    

                except Exception as e:

                    print(f"         ERROR Enhanced NN failed: {e}")
                ...
                if fnr_candidates:
                    ...
                    print(f"\n      OK WINNER: {best_model_info['name']} (R ={best_model_info['r2']:.3f})")
                    ...
                    print(f"      OK FNR scaler created ({len(X_train_surrogate.columns)} features)")
                    ...
                    print(f"      ERROR All approaches failed, falling back to standard training...")



            # [TARGET] ULTIMATE: Train TTD using MULTI-STRATEGY approach (try 3 methods, pick best)
            ...
            if target_metric == 'time_to_detection_threshold' and not is_classification:
                ...
                try:
                    ...
                    # OK FIX: Use SCALED targets for neural network training
                    ...
                    if hasattr(self, 'y_train_scaled') and target_metric in self.y_train_scaled:
                        ...
                        y_train_for_nn = y_train_target.values

                        y_test_for_nn = y_test_target.values

                        print(f"         [WARNING] Using unscaled targets")
                    ...
                    # OK FIX: Evaluate in ORIGINAL space
                ...
                # APPROACH 2: Gradient Boosting Ensemble (Log-Space for TTD)
                ...
                try:
                    ...
                    # For TTD, use log-space regression (handles wide range better)

                    if target_metric == 'time_to_detection_threshold':

                        y_train_for_gb = np.log1p(y_train_target.values)

                        y_test_for_gb = np.log1p(y_test_target.values)

                        use_log_space = True

                        print(f"         Using log-space regression (TTD-optimized)")

                    else:

                        y_train_for_gb = y_train_target.values

                        y_test_for_gb = y_test_target.values

                        use_log_space = False

                    

                    # LightGBM with better hyperparameters
                    ...
                    lgb_params = {
                        ...
                        'num_leaves': 127 if target_metric == 'time_to_detection_threshold' else 63,
                        ...
                        'min_data_in_leaf': 20 if target_metric == 'time_to_detection_threshold' else 10,

                        'verbose': -1,

                        'n_estimators': 1000,  # More trees for complex targets
                    ...
                    lgb_model.fit(
                        ...
                        X_train_surrogate, y_train_for_gb,

                        eval_set=[(X_test_surrogate, y_test_for_gb)],
                    ...
                    # XGBoost with optimized parameters for heavy-tailed targets
                    ...
                    # CatBoost for highly non-linear relationships
                    ...
                    cb_model.fit(X_train_surrogate, y_train_for_gb, eval_set=(X_test_surrogate, y_test_for_gb))

                    

                    # Ensemble: Weighted average (LGB + XGB + CB)
                    ...
                    # OK Use dynamic weighted average based on individual model correlations
                    ...
                    if use_log_space:
                        ...
                        y_test_space = y_test_for_gb

                    else:

                        y_test_space = y_test_target.values

                    

                    # Get weights based on correlation (stronger correlation = higher weight)

                    try:

                        lgb_corr = max(0.01, abs(pearsonr(lgb_pred, y_test_space)[0]))

                        xgb_corr = max(0.01, abs(pearsonr(xgb_pred, y_test_space)[0]))

                        cb_corr = max(0.01, abs(pearsonr(cb_pred, y_test_space)[0]))

                        total_corr = lgb_corr + xgb_corr + cb_corr

                        weights = np.array([lgb_corr, xgb_corr, cb_corr]) / total_corr

                        print(f"         Correlation-based weights: LGB={weights[0]:.2f}, XGB={weights[1]:.2f}, CB={weights[2]:.2f}")

                    except:

                        # Fallback to fixed weights if correlation fails
                    ...
                    if use_log_space:
                        ...
                        y_test_original = np.expm1(y_test_for_gb)

                    else:

                        ensemble_pred_original = ensemble_pred

                        y_test_original = y_test_for_gb
                    ...
                    if target_metric == 'time_to_detection_threshold':
                        ...
                        ensemble_pred_original = np.clip(ensemble_pred_original, 10, 9000)

                    

                    # OK DEBUG: Check prediction validity

                    if np.any(ensemble_pred_original < 0) or np.any(ensemble_pred_original > 15000):

                        print(f"         [WARNING]  Ensemble predictions out of range: [{ensemble_pred_original.min():.1f}, {ensemble_pred_original.max():.1f}]")
                    ...
                    ensemble_mse = np.mean((y_test_original - ensemble_pred_original)**2)

                    

                    # Create wrapper for ensemble

                    ensemble_model = EnsembleModelV2(

                        lgb_model, xgb_model, cb_model,

                        use_log=use_log_space,

                        clip_range=(10, 9000) if target_metric == 'time_to_detection_threshold' else None
                ...
                # APPROACH 3: Two-Stage TTD Predictor (Censoring-Aware)

                #                                                             

                print(f"\n      [TARGET] APPROACH 3: Two-Stage TTD (Censoring-Aware)...")

                try:

                    # Stage 1: Detect censored samples (TTD >= 9000 means timeout)

                    TIMEOUT_THRESHOLD = 8900  # Slightly below 9000 to catch near-timeouts

                    

                    y_train_censored = (y_train_target.values >= TIMEOUT_THRESHOLD).astype(int)

                    y_test_censored = (y_test_target.values >= TIMEOUT_THRESHOLD).astype(int)

                    

                    print(f"         Censored in train: {np.sum(y_train_censored)}/{len(y_train_censored)} ({100*np.mean(y_train_censored):.1f}%)")

                    print(f"         Censored in test: {np.sum(y_test_censored)}/{len(y_test_censored)} ({100*np.mean(y_test_censored):.1f}%)")

                    

                    # Train Stage 1: Censoring classifier

                    from sklearn.ensemble import RandomForestClassifier

                    censoring_classifier = RandomForestClassifier(
                    ...
                    censoring_classifier.fit(X_train_surrogate, y_train_censored)

                    

                    # Stage 2: Regressor ONLY on non-censored data

                    non_censored_train_mask = y_train_target.values < TIMEOUT_THRESHOLD

                    non_censored_count = np.sum(non_censored_train_mask)

                    

                    if non_censored_count > 100:  # Need sufficient non-censored samples

                        X_train_non_censored = X_train_surrogate[non_censored_train_mask]

                        y_train_non_censored = y_train_target.values[non_censored_train_mask]

                        

                        # Log-transform non-censored TTD (reduces skewness)

                        y_train_log = np.log1p(y_train_non_censored)

                        

                        # Train Stage 2: TTD regressor

                        import lightgbm as lgb

                        ttd_regressor = lgb.LGBMRegressor(
                        ...
                        ttd_regressor.fit(X_train_non_censored, y_train_log)

                        

                        # Create two-stage model wrapper

                        class TwoStageTTDModel:

                            """Picklable two-stage TTD predictor"""

                            def __init__(self, classifier, regressor, timeout_value=9000.0):
                            ...
                            def predict(self, X):
                                ...
                                # Stage 1: Predict censoring probability

                                censoring_proba = self.classifier.predict_proba(X_array)[:, 1]

                                

                                # Stage 2: Predict log(TTD) for all samples

                                ttd_log_pred = self.regressor.predict(X_array)

                                ttd_pred = np.expm1(ttd_log_pred)  # Inverse of log1p

                                

                                # Combine: High censoring probability -> timeout

                                # Use soft threshold for smoother predictions
                                ...
                                # Clip to valid range [10, 9000]

                                final_pred = np.clip(final_pred, 10, self.timeout_value)
                        ...
                        # Create and evaluate two-stage model

                        two_stage_model = TwoStageTTDModel(censoring_classifier, ttd_regressor)

                        two_stage_pred = two_stage_model.predict(X_test_surrogate)

                        

                        # Metrics

                        two_stage_r2 = r2_score(y_test_target.values, two_stage_pred)

                        two_stage_mse = np.mean((y_test_target.values - two_stage_pred)**2)

                        two_stage_rmse = np.sqrt(two_stage_mse)

                        

                        # Debug output

                        print(f"         Pred range: [{two_stage_pred.min():.1f}, {two_stage_pred.max():.1f}]")

                        print(f"         True range: [{y_test_target.values.min():.1f}, {y_test_target.values.max():.1f}]")

                        print(f"         Predicted timeouts: {np.sum(two_stage_pred >= 8900)}/{len(two_stage_pred)}")

                        print(f"         Actual timeouts: {np.sum(y_test_target.values >= 8900)}/{len(y_test_target.values)}")

                        

                        ttd_candidates['two_stage'] = {
                            ...
                            'name': 'Two-Stage TTD (Censoring-Aware)'

                        }

                        print(f"         OK Two-Stage: R ={two_stage_r2:.3f}, RMSE={two_stage_rmse:.1f}")

                    else:

                        print(f"         [WARNING] Insufficient non-censored samples ({non_censored_count} < 100)")

                

                except Exception as e:

                    print(f"         ERROR Two-Stage failed: {e}")
                ...
                # APPROACH 4: Enhanced Neural Network (Deeper + Regularization)

                #                                                             

                print(f"\n        APPROACH 4: Enhanced Deep Neural Network...")
                ...
                try:
                    ...
                    # OK CRITICAL FIX: Create y_scaler for neural network training

                    from sklearn.preprocessing import StandardScaler

                    y_scaler_nn = StandardScaler()

                    

                    # Get raw target values

                    y_train_raw = y_train_target.values

                    y_test_raw = y_test_target.values

                    

                    # Scale targets for neural network

                    y_train_for_nn = y_scaler_nn.fit_transform(y_train_raw.reshape(-1, 1)).flatten()

                    y_test_for_nn = y_scaler_nn.transform(y_test_raw.reshape(-1, 1)).flatten()

                    

                    # OK Store the y_scaler for later inverse transform during RL

                    if not hasattr(self, 'y_scalers'):

                        self.y_scalers = {}

                    self.y_scalers[target_metric] = y_scaler_nn

                    

                    print(f"         OK Created y_scaler for {target_metric}")

                    print(f"            mean={y_scaler_nn.mean_[0]:.4f}, scale={y_scaler_nn.scale_[0]:.4f}")

                    

                    # OK CRITICAL FIX: Use RobustScaler for TTD (robust to outliers)

                    from sklearn.preprocessing import RobustScaler

                    y_scaler_ttd = RobustScaler()

                    y_train_scaled = y_scaler_ttd.fit_transform(y_train_target.values.reshape(-1, 1)).flatten()

                    y_test_scaled = y_scaler_ttd.transform(y_test_target.values.reshape(-1, 1)).flatten()
                    ...
                    self.y_scalers[target_metric] = y_scaler_ttd

                    

                    print(f"         OK Using SCALED targets for TTD NN (RobustScaler)")

                    print(f"         Scaled range: [{y_train_scaled.min():.2f}, {y_train_scaled.max():.2f}]")

                    

                    # Prepare data with SCALED targets
                    ...
                    # OK FIX 1: CREATE DATALOADER (was missing!)
                    ...
                    # TTD has wide range [14-9000] - use WIDER network
                    ...
                    class EnhancedNN(nn.Module):
                        ...
                        def __init__(self, input_dim):
                            ...
                            # OK MUCH WIDER first layer for TTD's complexity
                    ...
                    criterion = nn.HuberLoss(delta=0.5)  # OK More sensitive to outliers for TTD

                    optimizer = optim.Adam(enhanced_nn.parameters(), lr=0.0005, weight_decay=1e-5)  # OK Slower learning rate

                    scheduler = optim.lr_scheduler.StepLR(optimizer, step_size=100, gamma=0.5)  # OK Step-based scheduler
                    ...
                    n_epochs = 500  # OK More epochs for TTD complexity
                    ...
                    # OK CRITICAL FIX: Always inverse transform since we now ALWAYS scale
                    ...
                    if hasattr(self, 'y_scalers') and target_metric in self.y_scalers:
                        ...
                        # OK FIX 2: Use TEST data, not TRAIN data!

                        y_test_true_original = y_test_target.values  # Correct!
                        ...
                        print(f"         OK NN predictions (original scale): [{test_pred_original.min():.1f}, {test_pred_original.max():.1f}]")

                        print(f"         OK Enhanced NN R  (original scale): {enhanced_r2:.3f}")

                    else:

                        print(f"         [WARNING] WARNING: No y_scaler found for {target_metric}!")

                        # Fallback: use scaled predictions directly (WRONG but won't crash)
                    ...
                    # OK CRITICAL: Wrap neural network for sklearn compatibility
                ...
                # SELECT BEST PERFORMER WITH VALIDATION
                ...
                if ttd_candidates:
                    ...
                    # OK CRITICAL FIX: Check if ANY model is actually good

                    if best_model_info['r2'] < 0.05:

                        print(f"\n      [WARNING]  WARNING: All TTD models have R  < 0.05 (worse than baseline!)")

                        print(f"      [DATA] Current best: {best_model_info['name']} with R ={best_model_info['r2']:.3f}")

                        print(f"        TTD is inherently difficult to predict from static features")

                        print(f"        Using ensemble as default despite poor performance")

                    

                    # OK CRITICAL: Check if ALL models are worse than baseline

                    if best_model_info['r2'] < 0.0:

                        print(f"\n      [WARNING]  ALL MODELS WORSE THAN BASELINE (R  < 0.0)!")

                        print(f"      [DATA] Best model: {best_model_info['name']} with R ={best_model_info['r2']:.3f}")

                        print(f"        TTD cannot be predicted from static features - using data statistics")

                        

                        # OK FALLBACK: Create constant predictor (predicts median/mean)

                        class ConstantPredictor:

                            """Predicts constant value (median of training data)"""
                        ...
                        median_ttd = np.median(y_train_target.values)

                        constant_model = ConstantPredictor(median_ttd)

                        constant_pred = constant_model.predict(X_test_surrogate)

                        constant_r2 = r2_score(y_test_target.values, constant_pred)

                        constant_mse = np.mean((y_test_target.values - constant_pred)**2)

                        

                        print(f"      OK Constant predictor (median={median_ttd:.1f}): R ={constant_r2:.3f}")

                        

                        # Use constant model instead

                        self.surrogate_models[target_metric] = constant_model

                        surrogate_performance[target_metric] = {

                            'test_r2': constant_r2,

                            'test_mse': constant_mse,

                            'type': 'regression',

                            'model_type': 'Constant Predictor (Median TTD)'
                    ...
                    print(f"      OK TTD scaler created ({len(X_train_surrogate.columns)} features)")
            ...
            if is_classification:
                ...
                print(f"   [DATA] Classification task detected")
                ...
                y_train_target.values = le.transform(y_train_target)

                y_test_target.values = le.transform(y_test_target)
                ...
                print(f"   [DATA] Regression task detected")
                ...
                if target_skewness > 1.5:
                    ...
                    try:
                        ...
                        if new_skewness < target_skewness:
                            ...
                            # OK FIXED: Create new Series instead of modifying .values

                            y_train_target = pd.Series(y_train_transformed, index=y_train_target.index, name=y_train_target.name)

                            y_test_target = pd.Series(y_test_transformed, index=y_test_target.index, name=y_test_target.name)

                            self.target_transformers[target_metric] = transformer

                            print("      OK Target transformation applied")

                        else:

                            y_train_target = pd.Series(y_train_numeric.values, index=y_train_target.index, name=y_train_target.name)

                            y_test_target = pd.Series(y_test_numeric.values, index=y_test_target.index, name=y_test_target.name)

                            self.target_transformers[target_metric] = None

                            print("      ERROR Target transformation didn't improve")

                            

                    except Exception as e:

                        print(f"      ERROR Target transformation failed: {e}")
                ...
                if target_metric == 'time_to_detection_threshold':
                    ...
                    timeout_count_train = (y_train_target.values >= TIMEOUT).sum()

                    timeout_count_test = (y_test_target.values >= TIMEOUT).sum()

                    

                    print(f"          TTD timeout values (9000):")

                    print(f"         Train: {timeout_count_train}/{len(y_train_target.values)} ({100*timeout_count_train/len(y_train_target.values):.1f}%)")

                    print(f"         Test: {timeout_count_test}/{len(y_test_target.values)} ({100*timeout_count_test/len(y_test_target.values):.1f}%)")

                    print(f"      OK Keeping timeout values as-is (valid data representing 'no detection')")
            ...
            try:
                ...
                X_train_selected = selector.fit_transform(X_train_surrogate, y_train_target.values)
                ...
                print(f"      OK Selected features (list): {len(selected_features)}")
                ...
                print(f"      OK Scaler fitted and stored for {target_metric}")
                ...
                print(f"          Feature selection failed: {e}, using fallback")
                ...
                print(f"      OK Fallback scaler stored for {target_metric}")
            ...
            print(f"      OK Stored feature mapping: {len(selected_features)} features") 

            print(f"      Top 5 features: {selected_features[:5]}")

            

            # OK CRITICAL VALIDATION: Check all features exist in main dataset

            missing_in_main = [f for f in selected_features if f not in self.X_train.columns]

            if missing_in_main:

                print(f"      [WARNING] WARNING: {len(missing_in_main)} features NOT in main X_train:")

                print(f"         {missing_in_main[:5]}...")

                print(f"         RL environment may have issues with these features!")
            ...
            try:
                ...
                print(f"   OK Using stratified {cv_folds}-fold CV (balanced across target range)")
                ...
                print(f"   [WARNING] Using regular {cv_folds}-fold CV (stratification failed)")



            X_train_np = X_train_final.values if hasattr(X_train_final, 'values') else X_train_final

            y_train_np = y_train_target.values if isinstance(y_train_target.values, np.ndarray) else np.array(y_train_target.values)
            ...
            if is_classification:
                ...
                if valid_performances:
                    ...
                    print(f"   [DATA] CV Results: Accuracy: {avg_acc:.4f}   {np.std([p['test_accuracy'] for p in valid_performances]):.4f}")

                    print(f"                 F1: {avg_f1:.4f}   {np.std([p['test_f1'] for p in valid_performances]):.4f}")

                else:

                    print("   ERROR All CV folds failed")
                ...
                if valid_performances:
                    ...
                    print(f"   [DATA] CV Results: R : {avg_r2:.4f}   {np.std([p['test_r2'] for p in valid_performances]):.4f}")
            ...
            try:
                ...
                final_model, final_performance = self._train_with_multiple_strategies(
                    ...
                    X_train_final, y_train_target.values, X_test_final, y_test_target.values,
                ...
                if is_classification:
                    ...
                    print(f"   OK Final Test Accuracy: {final_performance['test_accuracy']:.4f}")

                    

                    if final_performance['test_accuracy'] < 0.5:

                        print("       VERY LOW: Test accuracy < 0.5")
                        ...
                        print("   [DATA] MODERATE: Test accuracy < 0.85")
                    ...
                    if target_metric == 'time_to_detection_threshold' and hasattr(self, 'ttd_log_transform') and self.ttd_log_transform['enabled']:
                        ...
                        y_test_original = np.exp(y_test_target.values) - epsilon
                        ...
                        if 'test_normalized_mse' not in final_performance:
                            ...
                            final_performance['test_normalized_mse'] = self._calculate_normalized_mse(y_test_target.values, test_pred)

                    

                    print(f"   OK Final Test R : {final_performance['test_r2']:.4f}")

                    print(f"   OK Final Test MSE: {final_performance['test_mse']:.4f}")
                    ...
                    if 'test_normalized_mse' in final_performance:
                        ...
                        print(f"   [DATA] Normalized MSE by variance: {norm_mse['mse_normalized_by_variance']:.4f}")

                        print(f"   [DATA] RMSE normalized by std: {norm_mse['rmse_normalized_by_std']:.4f}")

                        print(f"   [DATA] RMSE normalized by range: {norm_mse['rmse_normalized_by_range']:.4f}")

                    

                    if final_performance['test_r2'] < 0.0:

                        print("       VERY POOR: Test R  < 0.0")

                    elif final_performance['test_r2'] < 0.3:

                        print("   ERROR POOR: Test R  < 0.3")
                        ...
                        print("   [DATA] MODERATE: Test R  < 0.8")
                ...
                print(f"   ERROR Final model training failed: {e}")
            ...
            print(f"OK Stored UNIQUE model for {target_metric} (ID: {model_id})")
        ...
        if 'false_negative_rate' in self.y_train.columns and 'time_to_detection_threshold' in self.y_train.columns:
            ...
            if abs(fnr_ttd_corr) > 0.90:
                ...
                print("[WARNING]  WARNING: FNR and TTD are nearly identical (correlation > 0.9)!")
                ...
                print("OK FNR and TTD have healthy correlation (<0.7)")
        ...
        for metric, model in self.surrogate_models.items():
            ...
            if metric in self.surrogate_features:
                ...
                print(f"   OK Saved feature mapping for {metric}")
        ...
        print(f"OK Surrogate configurations saved to: {config_path}")
        ...
        print(f"OK Successfully trained: {len(successful_targets)}/{len(self.y_train.columns)} targets")

        

        if skipped_targets:

            print(f"    Skipped targets ({len(skipped_targets)}): {skipped_targets}")

        

        if regression_targets:

            print(f"\n[DATA] Regression targets ({len(regression_targets)}):")
        ...
        print("OK Enhanced surrogate models ready for RL!")



        # Fidelity validation

        print("\n[TARGET] SURROGATE MODEL FIDELITY VALIDATION")
        ...
        for target, perf in surrogate_performance.items():
            ...
            if perf['type'] == 'regression':
                ...
                if r2 < 0.90:
                    ...
                    print(f"    {target}: R  = {r2:.4f} < 0.90 (BELOW THRESHOLD)")

                elif r2 < 0.95:

                    print(f"OK {target}: R  = {r2:.4f} (Acceptable)")
        ...
        # OK FIXED: Actually check if models meet threshold
        ...
        if poor_models:
            ...
            print(f"\n[WARNING]  WARNING: {len(poor_models)} model(s) below R  = 0.90 threshold:")

            for model in poor_models:

                r2 = surrogate_performance[model]['test_r2']

                print(f"   ERROR {model}: R  = {r2:.4f}")

            print("\n[WARNING]  CRITICAL: RL performance will be LIMITED by poor surrogate models!")
            ...
            print("\nOK All regression surrogates meet minimum R    0.90 threshold!")



        # OK FINAL VALIDATION: Ensure all targets have scalers
            ...
            if target in self.surrogate_models:
                ...
                if target in self.surrogate_metric_scalers:
                    ...
                    if hasattr(scaler, 'mean_'):
                        ...
                        print(f"OK {target}: Scaler OK ({len(scaler.mean_)} features)")

                    elif hasattr(scaler, 'center_'):

                        print(f"OK {target}: Scaler OK ({len(scaler.center_)} features)")

                    else:

                        print(f"ERROR {target}: Scaler exists but not fitted!")

                else:

                    print(f"ERROR {target}: NO SCALER! (Model exists but scaler missing)")
                ...
                if target in self.surrogate_features:
                    ...
                    print(f"   [WARNING] WARNING: No feature list stored for {target}")
    ...
    def _create_physics_based_ttd_predictor(self, X_train, y_train, X_test, y_test):
        ...
        OK Physics-based TTD predictor using analytical formulas
        ...
        OK FIXED: Uses module-level PhysicsTTDPredictor class for pickle support
        ...
        # OK Use module-level PhysicsTTDPredictor class (defined at top of file)
        ...
        print(f"      OK Physics TTD: Train R ={train_r2:.3f}, Test R ={test_r2:.3f}")
    ...
    def _create_component_based_fnr_predictor(self, X_train, y_train, X_test, y_test):
        ...
        try:
            ...
            print(f"      [DATA] Component 1: SNR Penalty")
            ...
            print(f"      [DATA] Component 2: Threshold Factor")
            ...
            print(f"      [DATA] Component 3: Circuit Reliability")
            ...
            print(f"      [DATA] Component 4: Environmental Noise")
            ...
            print(f"         OK SNR Penalty R  = {snr_r2:.4f}")
            ...
            print(f"         OK Threshold Factor R  = {threshold_r2:.4f}")
            ...
            print(f"         OK Circuit Factor R  = {circuit_r2:.4f}")
            ...
            print(f"         OK Environmental Noise R  = {env_r2:.4f}")
            ...
            print(f"      OK Combined FNR R  = {overall_r2:.4f}")

            print(f"      OK Combined FNR MSE = {overall_mse:.6f}")
            ...
            print(f"      OK Component-based FNR predictor created!")
            ...
            if overall_r2 < 0.5:
                ...
                print(f"      [WARNING] WARNING: R  < 0.5, consider using standard ensemble instead")
            ...
            print(f"      ERROR Component-based FNR failed: {e}")
    ...
    def check_data_leakage(self, metric_name):
        ...
        if test_min < train_min:
            ...
            if diff_pct > 5:  # Only warn if >5% beyond range
                ...
                print(f"   [DATA] INFO: {metric_name} test_min ({test_min:.4f}) slightly below train_min ({train_min:.4f})")
        ...
        if test_max > train_max:
            ...
            if diff_pct > 5:  # Only warn if >5% beyond range
                ...
                print(f"   [DATA] INFO: {metric_name} test_max ({test_max:.4f}) slightly above train_max ({train_max:.4f})")
    ...
    def setup_rl_environment(self, target_metrics: List[str] = None) -> gym.Env:
        ...
        class BiosensorEnv(gym.Env):
            ...
            def __init__(self, modifiable_bounds, fixed_values, surrogate_models, 
                ...
                #    "  FIX #1: Validate and store data_min_max FIRST (before anything else)

                if not data_min_max:

                    raise ValueError("   ' CRITICAL: data_min_max is empty! Cannot create environment.")

                

                if not isinstance(data_min_max, dict):

                    raise TypeError(f"   ' data_min_max must be dict, got {type(data_min_max)}")
                ...
                if missing:
                    ...
                    raise ValueError(f"   ' Missing required metrics in data_min_max: {missing}")

                

                print(f"      Environment {id(self)}: data_min_max stored with {len(self.data_min_max)} metrics")
                ...
                self.y_scalers = y_scalers or {}  # OK ADD THIS LINE
                ...
                if surrogate_metric_scalers:
                    ...
                    for metric, scaler in surrogate_metric_scalers.items():
                        ...
                        if scaler is not None:
                            ...
                            # OK Validate scaler is fitted (check both StandardScaler and RobustScaler)
                ...
                if not data_min_max:
                    ...
                    raise ValueError("   data_min_max parameter is empty!")
                ...
                if not hasattr(BiosensorEnv, '_data_range_confirmed'):
                    ...
                    print(f"      data_min_max: {len(self.data_min_max)} metrics")
                ...
                #    FIX: Simplified action space - only 3 actions per feature (up/down/keep)
                ...
                print(f"      Simplified action space: {len(self.action_mapping)} actions")
                ...
                self.action_space = gym_spaces.Box(
                    ...
                    shape=(total_action_dim,),  # OK CORRECT - 9 features
                ...
                print(f"   OK Action space created: {self.action_space}")



                print(f"   OK Continuous action space: {self.action_space.shape[0]} dimensions")



                #    "  FIX: Calculate exact observation size based on _get_observation implementation
                ...
                print(f"   OK Observation space: {obs_size} dimensions")
                ...
                print(f"   Environment initialized: {self.n_modifiable} modifiable parameters")
            ...
            def _validate_surrogate_models(self):
                ...
                for metric, model in self.surrogate_models.items():
                    ...
                    try:
                        ...
                        # OK FIXED: Always use .predict() for consistency
                        ...
                        if np.isnan(test_output).any():
                            ...
                            print(f"   Surrogate model {metric} validated ({model_type})")
            ...
            def _create_full_state(self, target_metric=None):
                ...
                # OK DEBUG: Track what we're filling
                ...
                for idx, feature in enumerate(feature_order):
                    ...
                    if feature in self.modifiable_features:
                        ...
                        # OK CRITICAL: Feature not found - fill with 0 and warn
                ...
                # OK DEBUG: Print FIRST call only to see what's being fed
            ...
            def _initialize_log_files(self):
                ...
                with open(episode_log_path, 'w', newline='') as f:
                    ...
                    writer.writerow([
                        ...
                        'episode', 'mean_reward', 'total_reward', 'best_reward',  #    Fixed order
            ...
            def reset(self, seed=None, options=None):
                ...
                # OK IMPROVED: Curriculum-based initialization strategy
                ...
                if init_strategy == 'random' or not hasattr(self, 'best_states'):
                    ...
                    for i, feature in enumerate(self.modifiable_features):
                        ...
                        if feature in self.X_train.columns:
                            ...
                            # OK Use 80% of range (avoid extreme edges)
                    ...
                    # OK CORRECTED: Check if best_states exists and has elements
                ...
                # OK FIXED: Return Gymnasium-style (obs, info)
            ...
            def _get_observation(self):
                ...
                # OK STEP 1: Normalize modifiable features using ORIGINAL data distribution
                ...
                # OK STEP 2: Calculate current predictions for all targets
                ...
                for metric in ['signal_to_noise_ratio_SNR', 'dynamic_range_of_output', 
                    ...
                    if metric in self.surrogate_models:
                        ...
                        try:
                            ...
                            # OK FIXED: Always use .predict() for all models

                            pred = model.predict(full_state.reshape(1, -1))[0]

                            

                            # OK Use data_min_max for proper normalization
                ...
                # OK STEP 3: Calculate target gaps (how much improvement is possible)
                ...
                # OK STEP 4: Context information (episode progress and recent performance)
                ...
                # OK STEP 5: Bounds utilization (how close to limits)
                ...
                try:
                    ...
                    print(f"[WARNING] Observation construction failed: {e}")
                ...
                # OK CRITICAL: Validate observation size
            ...
            def step(self, action):
                ...
                # OK Handle continuous actions (action is a numpy array of floats in [-1, 1])
                ...
                # OK CORRECTED: Apply continuous actions properly
                ...
                # OK FIXED: Apply actions in ORIGINAL space
                ...
                for i, feature in enumerate(self.modifiable_features):  # All features, not just continuous
                    ...
                    # OK IMPROVED: Smooth action scaling with stochastic exploration

                    episode_progress = self.step_count / self.max_steps



                    # OK FIXED: Adaptive action scaling - start large, decrease gradually
                ...
                # OK SAFETY: Additional clipping to prevent overflow

                reward = float(np.clip(reward, -10.0, 10.0))



                # OK SAFETY: Check for NaN/Inf

                if not np.isfinite(reward):

                    print(f"[WARNING] Non-finite reward in step()! Resetting to -5.0")
                ...
                # OK CLEAN EPISODE SUMMARY (only print once per environment)
                ...
                # OK FIXED: Return Gymnasium-style (5 values)
            ...
            def _calculate_reward_with_predictions(self):
                ...
                try:
                    ...
                    # OK VALIDATION: Ensure features are lists, not pandas Index
                    ...
                    if not hasattr(self, '_features_validated'):
                        ...
                        for metric in self.surrogate_features:
                            ...
                            if not isinstance(self.surrogate_features[metric], list):
                                ...
                                print(f"   [WARNING] Converting {metric} features to list")
                    ...
                    for metric in self.target_metrics:
                        ...
                        try:
                            ...
                            # OK CRITICAL FIX: Get the CORRECT features for THIS metric

                            if metric not in self.surrogate_features:

                                print(f"[WARNING] No feature mapping for {metric}!")
                            ...
                            # OK Create state with ONLY the features this model was trained on
                            ...
                            for idx, feature in enumerate(feature_names):
                                ...
                                if feature in self.modifiable_features:
                                    ...
                                    #    Feature not in modifiable/fixed - use 0 as default

                                    full_state[idx] = 0.0

                                    # Silently fill with 0 - engineered features handled this way



                            # OK CRITICAL FIX: Apply metric-specific scaling BEFORE prediction
                            ...
                            # OK Predict using the model

                            pred_scaled = model.predict(full_state_scaled.reshape(1, -1))[0]



                            # OK CRITICAL FIX: Inverse transform for NNWrapper (trained on scaled targets)
                            ...
                            if type(model).__name__ == 'NNWrapper':
                                ...
                                if hasattr(self, 'y_scalers') and metric in self.y_scalers:
                                    ...
                                    try:
                                        ...
                                        if metric == 'false_negative_rate':
                                            ...
                                            if pred_original < 0 or pred_original > 0.5:
                                                ...
                                                if debug_step:
                                                    ...
                                                    print(f"      [WARNING] FNR out of realistic bounds!")
                                        ...
                                        print(f"      ERROR Failed to inverse transform {metric}: {e}")
                                    ...
                                    if debug_step:
                                        ...
                                        print(f"   [WARNING] No y_scaler for {metric}!")
                            ...
                            if self.step_count == 1:  # Only show errors on first step
                                ...
                                print(f"[WARNING] Failed to predict {metric}: {e}")
                    ...
                    # OK FIXED: Use STRICT clipping to training ranges
                    ...
                    if hasattr(self, 'data_min_max') and self.data_min_max:
                        ...
                        # OK FIXED: Single normalization WITHOUT over-clipping
                    ...
                    # OK Map [0, 1] -> [0, 10] with clear scaling
                    ...
                    # OK Milestone bonuses for exceptional performance
                    ...
                    # OK BETTER LOGGING: Show raw values too

                    if self.step_count % 50 == 0:

                        print(f"\n[DATA] REWARD CHECK @ Step {self.step_count}:")
                        ...
                        if self.step_count == 50:
                            ...
                            print(f"   [WARNING]  If TTD shows 20-30 instead of 2000-3000, TTD model is BROKEN!\n")
                    ...
                    # OK Improvement tracking (for logging only, NOT added to reward)
                    ...
                    # OK FINAL REWARD: Just the base reward (no confusing bonuses)
            ...
            def _log_step_details(self, action, reward, predictions):
                ...
                try:
                    ...
                    print(f"               Failed to log step details: {e}")
                ...
                try:
                    ...
                    with open(trajectory_log_path, 'a', newline='') as f:
                        ...
                        if len(row) != expected_length:
                            ...
                            print(f"               Row length mismatch: got {len(row)}, expected {expected_length}")
                    ...
                    print(f"               Failed to log trajectory: {e}")
                    ...
                    if self.current_episode % 50 == 0:  # Log every 50 episodes
                        ...
                        if len(self.current_episode_rewards) >= 10:
                            ...
                            if abs(recent_trend) < 0.001:
                                ...
                                print(f"                WARNING: Possible learning stagnation (trend: {recent_trend:.6f})")
        ...
        #    "  FIX: Calculate ranges from ACTUAL training data (self.y_train)
        ...
        for metric in mos_metrics:
            ...
            if metric in self.y_train.columns:
                ...
                # OK DEBUG: Print actual data ranges

                if metric == 'signal_to_noise_ratio_SNR':

                    print(f"   [DATA] {metric} ACTUAL data: min={actual_min:.4f}, max={actual_max:.4f}")
                ...
                if max_val <= min_val:
                    ...
                    print(f"   [WARNING]  {metric} has invalid range, using safe defaults")
                    ...
                    if 'snr' in metric.lower():
                        ...
                        min_val, max_val = 0, 0.3  # OK FIX: Use realistic max based on actual data
                ...
                print(f"          {metric} not in y_train, using defaults")
        ...
        #    "  CRITICAL: Store at pipeline level BEFORE environment creation

        self.data_min_max_global = dict(data_min_max)

        print(f"\n   Stored data_min_max globally: {len(self.data_min_max_global)} metrics")
        ...
        for metric, default_range in required_defaults.items():
            ...
            if metric not in data_min_max:
                ...
                print(f"                Using default range for {metric}: {default_range}")
        ...
        if len(valid_target_metrics) < len(target_metrics):
            ...
            print(f"                WARNING: Some target metrics have no surrogate model: {missing}")
        ...
        #  "' CRITICAL VALIDATION: Ensure all scalers are valid BEFORE creating environments

        print("[*] Pre-flight check: Validating scalers...")
        ...
        if invalid_metrics:
            ...
            raise RuntimeError(
                ...
                f"   CRITICAL: Invalid scalers detected before RL training:\n"
            ...
            print(f"      All {len(target_metrics)} metric scalers validated")
        ...
        #    "  ADDITIONAL FIX: Store scalers at PIPELINE level for reference
        ...
        print(f"      Backed up {len(self.rl_scalers_backup)} scalers at pipeline level")
        ...
        try:
            ...
            print(f"         Creating {n_envs} vectorized environments (DummyVecEnv)...")
            ...
            def make_env(rank=0, seed=0):
                ...
                def _init():
                    ...
                    #    "  FIX: Create deep copies AND validate before environment creation
                    ...
                    if not data_min_max:
                        ...
                        raise ValueError(f"   ' Cannot create environment: data_min_max is empty in factory!")
                    ...
                    if not _local_data_min_max:
                        ...
                        raise ValueError(f"   ' Deep copy of data_min_max failed!")

                    

                    if len(_local_data_min_max) != len(data_min_max):

                        raise ValueError(f"   ' Deep copy corrupted data_min_max: {len(_local_data_min_max)} vs {len(data_min_max)}")
                    ...
                    # OK CORRECTED: Robust scaler storage with validation
                        ...
                        for metric, scaler in self.surrogate_metric_scalers.items():
                            ...
                            if scaler is not None and hasattr(scaler, 'center_'):
                                ...
                                try:
                                    ...
                                    print(f"[WARNING] Failed to copy scaler for {metric}: {e}")
                                    ...
                                    if metric in self.surrogate_features and hasattr(self, 'X_train'):
                                        ...
                                        if all(f in self.X_train.columns for f in features):
                                            ...
                                            print(f"   OK Rebuilt scaler for {metric}")

                    

                    # Create environment with copied data

                    # OK CRITICAL: Deep copy y_scalers for each environment
                    ...
                    env = BiosensorEnv(
                        ...
                        y_scalers=_local_y_scalers  # OK ADD THIS LINE
            ...
            print("[*] VALIDATION: Testing environment data_min_max...")
            ...
            if hasattr(self.rl_env, 'envs'):
                ...
                for i, env in enumerate(self.rl_env.envs[:2]):  # Check first 2
                    ...
                    if not hasattr(env, 'data_min_max') or len(env.data_min_max) == 0:
                        ...
                        raise RuntimeError(f"   Environment {i} has empty data_min_max after creation!")

                    print(f"      Env {i}: {len(env.data_min_max)} metrics stored")

            else:

                if not hasattr(self.rl_env, 'data_min_max') or len(self.rl_env.data_min_max) == 0:

                    raise RuntimeError("   Single environment has empty data_min_max!")
            ...
            print(f"       Could not create vectorized environments: {e}")
            ...
            # OK CRITICAL: Deep copy y_scalers
        ...
        try:
            ...
            try:
                ...
                if isinstance(info, list):
                    ...
                    print(f"               Unexpected info type: {type(info)}, attempting to extract predictions...")
                ...
                print(f"               Could not extract predictions: {e}")
            ...
            if not reward_is_scalar or not done_is_bool:
                ...
                print(f"             WARNING: Non-scalar reward or done - this may cause training issues")
    ...
    def get_stable_training_configs(self):
        ...
        ppo_config = {
            ...
            # OK OPTIMIZED for continuous action biosensor task

            "learning_rate": self._cosine_schedule(3e-4, 5e-6),  # OK Lower start, slower decay

            "n_steps": 2048,              # OK MORE data per update (was 2048)

            "batch_size": 128,            # OK Smaller batches for better gradients (was 512)

            "n_epochs": 20,               # OK MORE gradient updates per batch (was 10)

            "gamma": 0.99,                # OK Standard discount (0.995 too high)

            "gae_lambda": 0.95,           # OK Standard GAE (0.98 too high)

            "clip_range": 0.1,  # TIGHTER clipping for stability

            

            "ent_coef": 0.02,             # OK MUCH HIGHER for exploration (was 0.005!!!)
            ...
            "policy_kwargs": {
                ...
                "net_arch": dict(
                    ...
                    pi=[256, 256, 128],   # OK Slightly smaller but deeper (was [512, 256, 128])

                    vf=[256, 256, 128]    # OK Match policy network

                ),

                "activation_fn": torch.nn.Tanh,  # OK Tanh better for continuous (was ReLU)
    ...
    def _quick_eval(self, agent, n_episodes=10):
        ...
        for _ in range(n_episodes):
            ...
            while not done and step_count < max_steps:
                ...
                # OK FIXED: Handle both Gym and Gymnasium API
    ...
    def train_rl_agents(self, total_timesteps: int = 3000000):  # 3M minimum
        ...
        if self.rl_env is None:
            ...
            print("            Setting up RL environment first...")
        ...
        print(f"OK Reward variance test:")
        ...
        if np.mean(reward) < -0.5:
            ...
            print("[WARNING]  WARNING: Average reward is very negative!")

            print("   Check reward normalization ranges")

        elif np.mean(reward) > 0.0:

            print("OK Good! Positive average reward - agent can learn")
        ...
        print(f"OK Action space variance: {action_variance.mean():.4f}")
        ...
        for metric in test_env.surrogate_models.keys():
            ...
            try:
                ...
                print(f"ERROR Prediction failed for {metric}: {e}")



        print(f"OK Surrogate predictions:")
        ...
        try:
            ...
            print(f"      OK PPO initialized for continuous action space: {self.rl_env.action_space}")
                ...
                for i in range(10):
                    ...
                    if i % 2 == 1:
                        ...
                        if patience_counter >= patience:
                            ...
                            print(f"             Early stopping triggered (no improvement for {patience} checks)")
        ...
        if ppo_agent is not None:
            ...
            try:
                ...
                # OK FIXED: Longer training with better checkpointing
                ...
                for i in range(30):
                    ...
                    try:
                        ...
                        print(f"      ERROR Training failed at chunk {i+1}: {e}")
                    ...
                    if (i + 1) % 5 == 0:
                        ...
                        try:
                            ...
                            if eval_reward > best_eval_reward + 0.01:
                                ...
                                print(f"         OK New best! Saved checkpoint.")
                            ...
                            if patience_counter >= patience and i >= 18:
                                ...
                                print(f"      [WARNING] Early stopping (no improvement for {patience} checks)")
        ...
        if ppo_agent is not None:
            ...
            try:
                ...
                #    FIX: Simplified evaluation for vectorized environments
                ...
                while episodes_completed < n_eval_episodes:
                    ...
                    for step in range(300):
                        ...
                        #    FIX: Handle both scalar and array rewards safely
        ...
        if agents_performance:
            ...
            if best_performance >= 0.70:
                ...
                print("            RL optimization showed moderate success.")
                ...
                print("   1.             VERIFY surrogate model quality (R           0.90)")
        ...
        try:
            ...
            print(f"             Could not close environment cleanly: {e}")
    ...
    def plot_feature_importance_for_sclerostin(self):
        ...
        if not self.feature_importance:
            ...
            print("             No feature importance data available. Training a model to generate it...")
        ...
        print(f"              Sclerostin feature importance plot saved")
    ...
    def plot_sclerostin_correlation_matrix(self):
        ...
        print(f"              Sclerostin correlation matrix saved")
    ...
    def _save_ml_models_cache(self):
        ...
        print("\n '  Caching ML models for future use...")
        ...
        try:
            ...
            if not hasattr(self, 'surrogate_features') or not self.surrogate_features:
                ...
                print("      Cannot cache models - surrogate_features missing!")
            ...
            print(f"[*] Caching features for {len(self.surrogate_features)} metrics")

            

            for metric, model in self.surrogate_models.items():

                # OK Handle NNWrapper (which wraps PyTorch models)
            ...
            # OK CRITICAL FIX: Store scalers with validation
            ...
            if hasattr(self, 'surrogate_metric_scalers'):
                ...
                for metric, scaler in self.surrogate_metric_scalers.items():
                    ...
                    if scaler is None:
                        ...
                        print(f"   ERROR Scaler for {metric} is None!")
                    ...
                    if not (hasattr(scaler, 'mean_') or hasattr(scaler, 'center_')):
                        ...
                        print(f"   ERROR Scaler for {metric} not fitted!")

                        continue

                    

                    # OK Store the actual fitted scaler

                    scalers_cache['surrogate_metric_scalers'][metric] = scaler

                    n_features = len(scaler.mean_) if hasattr(scaler, 'mean_') else len(scaler.center_)

                    print(f"   OK Cached scaler for {metric} (fitted on {n_features} features)")
                ...
                print(f"   ERROR No surrogate_metric_scalers attribute found!")



            print(f"[*] Cached {len(scalers_cache['surrogate_metric_scalers'])} valid scalers")
            ...
            print("      ML models cached")

            

            print(f"[*] Cached models saved to: {saved_ml_dir}")
    ...
    def _load_ml_models_cache(self):
        ...
        #  "  CRITICAL: Ensure we have training data for scaler rebuilding

        if not hasattr(self, 'X_train') or self.X_train is None:

            print("          No X_train available - scalers cannot be rebuilt if corrupted")
            ...
            if hasattr(self, 'processed_data') and 'X' in self.processed_data:
                ...
                print("      Training data reconstructed for scaler rebuilding")
        ...
        try:
            ...
            print(f"      Loaded feature mappings for {len(self.surrogate_features)} metrics")
            ...
            # OK CRITICAL FIX: Load scalers directly (they're already fitted objects)
            ...
            for metric, scaler in cached_scalers.items():
                ...
                # OK FIXED: Check for BOTH StandardScaler (mean_) and RobustScaler (center_)
                ...
                if scaler is not None:
                    ...
                    if has_standard or has_robust:
                        ...
                        # OK It's a fitted scaler - use it directly!
                        ...
                        print(f"   OK Loaded fitted {scaler_type} for {metric} ({n_features} features)")

                    

                    elif isinstance(scaler, dict):

                        # OK It's scaler parameters stored as dict - reconstruct
                        ...
                        if 'mean_' in scaler:
                            ...
                            print(f"   OK Reconstructed StandardScaler for {metric}")
                            ...
                            print(f"   OK Reconstructed RobustScaler for {metric}")

                        

                        else:

                            print(f"   ERROR Invalid dict format for {metric}: missing mean_ or center_")
                        ...
                        print(f"   ERROR Invalid scaler format for {metric}: {type(scaler)}")
            ...
            # OK CRITICAL: Verify all loaded scalers

            print(f"   [DATA] Verification: {len(self.surrogate_metric_scalers)} scalers loaded")



            if len(self.surrogate_metric_scalers) == 0:

                print(f"   ERROR CRITICAL: No scalers loaded! Cache is broken!")
            ...
            for metric in self.target_metrics:
                ...
                if metric not in self.surrogate_metric_scalers:
                    ...
                    print(f"   ERROR Missing scaler for target metric: {metric}")
                ...
                # OK FIXED: Check for BOTH StandardScaler and RobustScaler
                ...
                if not (has_mean or has_center):
                    ...
                    print(f"   ERROR Scaler for {metric} is not fitted!")
                ...
                print(f"   OK Verified {metric}: {n_features} features, center range [{center.min():.3f}, {center.max():.3f}]")
            ...
            # OK CRITICAL: Verify all target metrics have scalers (NO EMERGENCY REBUILD)
            ...
            if missing_scalers:
                ...
                print(f"   ERROR CRITICAL: Missing scalers for: {missing_scalers}")

                print(f"   ERROR Cannot use cached models - scalers are corrupted!")

                print(f"   ERROR You must retrain models to regenerate scalers.")

                return False  # Force retraining



            print(f"   OK All target metrics have valid scalers")
            ...
            for metric, model_data in surrogate_cache['models'].items():
                ...
                if model_data['type'] == 'nn_wrapper':
                    ...
                    # OK Reconstruct NNWrapper model
                    ...
                    try:
                        ...
                        print(f"   OK NNWrapper model {metric} reconstructed (dim={input_dim})")

                    

                    except Exception as e:

                        print(f"   ERROR Failed to reconstruct NNWrapper {metric}: {e}")
                    ...
                    try:
                        ...
                        # OK Create fresh model instance
                        ...
                        try:
                            ...
                            print(f"   OK PyTorch model {metric} reconstructed (dim={input_dim})")

                        except RuntimeError as e:

                            print(f"   [WARNING] State dict mismatch for {metric}, trying non-strict load...")

                            model.load_state_dict(model_data['state_dict'], strict=False)

                            print(f"   [WARNING] PyTorch model {metric} loaded with warnings")
                        ...
                        print(f"   ERROR Failed to reconstruct PyTorch {metric}: {e}")
                    ...
                    # OK Reconstruct PhysicsBasedFNRPredictor

                    # This requires the component models - reconstruct from cached components

                    print(f"   [WARNING] PhysicsBasedFNR {metric} requires component models - will need retraining")
                    ...
                    # OK Reconstruct PhysicsTTDPredictor
                    ...
                    print(f"   OK PhysicsTTD model {metric} reconstructed")
                    ...
                    print(f"   OK sklearn model {metric} loaded")



                    # OK Also load cached feature mappings if available
                    ...
                    if features_path.exists():
                        ...
                        print(f"   OK Loaded feature mapping: {len(self.surrogate_features[metric])} features")

            

            print("      Surrogate models loaded")
            ...
            print("       All cached models loaded successfully!")
    ...
    def run_complete_pipeline(self, biomarker: Optional[str] = None, 
        ...
        if use_cached_models is None:
            ...
            try:
                ...
                # Non-interactive mode: default to training new models

                print("   (Non-interactive mode: defaulting to option 2)")
        ...
        if use_cached_models and not cached_models_available:
            ...
            print("          No cached models found. Will train new models.")
        ...
        try:
            ...
            print("[*] STAGE 1: Data Preprocessing & Feature Selection")
            ...
            # OK INITIALIZE surrogate_features if it doesn't exist

            if not hasattr(self, 'surrogate_features'):

                print("[WARNING] surrogate_features not initialized yet - will be populated during training")
            ...
            for metric in self.surrogate_models:
                ...
                print(f"OK {metric}: unique model (ID: {model_id})")
            ...
            # OK DIAGNOSTIC: Check if feature mappings are truly different
            ...
            if not models_loaded_from_cache:
                ...
                print("\n   STAGE 2-3: Using cached ML models (skipped training)")
            ...
            #  "  DIAGNOSTIC: Test scaler storage

            print("[*] DIAGNOSTIC: Checking scaler storage...")

            if hasattr(self, 'surrogate_metric_scalers'):

                print(f"      surrogate_metric_scalers exists with {len(self.surrogate_metric_scalers)} scalers")
                ...
                print(f"      surrogate_metric_scalers attribute missing!")



            #  "  DIAGNOSTIC: Test one environment's data_min_max

            if hasattr(self.rl_env, 'envs'):

                test_env = self.rl_env.envs[0]

                print(f"[*] DIAGNOSTIC: Test environment data_min_max...")
            ...
            #    "  FIX: Use standard training instead of buggy curriculum learning
            ...
            if not self.feature_importance:
                ...
                print("[*] STAGE 2: Training Supervised Models for Feature Importance")
                ...
                try:
                    ...
                    print(f"               Supervised model training failed: {e}")
            ...
            print("              Sclerostin-specific visualizations completed")
...
def main():
    ...
    with DualLogger(log_dir="logs"):
        ...
        print(f"\n         Sclerostin Biosensor RL Optimization")
        ...
        try:
            ...
            if results:
                ...
                print(f"[*] Sclerostin biosensor optimization completed successfully!")
                ...
                if 'model_performance' in results:
                    ...
                    for target, models in results['model_performance'].items():
                        ...
                        print(f"      Best: {best_model[0]} (R     = {best_model[1]['test_r2']:.4f})")
                ...
                if 'rl_performance' in results and results['rl_performance']:
                    ...
                    for agent, perf in results['rl_performance'].items():
                        ...
                        print(f"      Mean Reward: {perf['mean_reward']:.4f}      {perf['std_reward']:.4f}")

                        print(f"      Max Reward: {perf['max_reward']:.4f}")

            else:

                print(f"\n     ' Sclerostin biosensor optimization failed!")
