...
class RLAnalysisReport:
    ...
    def _analyze_feature_evolution(self):
        ...
        print(self.trajectory_df is None, self.config is None)
        ...
        for feature in feature_names:
            if f'{feature}_value' in self.trajectory_df.columns:
                feature_data = self.trajectory_df[f'{feature}_value']
                
                # Calculate feature statistics
                initial_value = feature_data.iloc[0] if len(feature_data) > 0 else 0
                final_value = feature_data.iloc[-1] if len(feature_data) > 0 else 0
                ...
                pct_change = (total_change / initial_value * 100) if initial_value != 0 else 0
    ...
    def _analyze_model_strategy(self):
        ...
        if len(self.step_df) > 1:
            reward_trend = np.polyfit(range(len(self.step_df)), self.step_df['reward'], 1)[0]
    ...
    def _create_visualizations(self, output_dir):
        ...
        if self.episode_df is not None:
        ...
        # 2. Feature Evolution
        if self.trajectory_df is not None and self.config is not None:
            feature_names = list(self.config['modifiable_bounds'].keys())
            ...
            if n_features > 0:
                ...
                for i, feature in enumerate(feature_names[:4]):  # Show first 4 features
                    if i < len(axes) and f'{feature}_value' in self.trajectory_df.columns:
                        feature_data = self.trajectory_df[f'{feature}_value']
...
if __name__ == "__main__":
    # Initialize analyzer with your log directory
    analyzer = RLAnalysisReport("rl_logs_20250708_105930")  # Replace with your actual log directory
